...
async def debug_vector_issues():
    ...
    try:
        ...
        try:
            ...
            print(f"{Fore.GREEN}✓ Vector extension appears to be working{Style.RESET_ALL}")
        except:
    ...
    try:
        ...
        # Retrieve and check
        retrieve_result = supabase.table("content_chunks").select("*").eq("source_id", source_id).execute()
        ...
        if retrieve_result.data:
            ...
            if "chunk_embedding" in chunk:
                ...
                if isinstance(embedding_data, str):
                    print(f"  Embedding is stored as string, length: {len(embedding_data)}")
    ...
    try:
        ...
        if search_result.data is None:
            print(f"{Fore.YELLOW}  Search returned None (might be normal if no data){Style.RESET_ALL}")
