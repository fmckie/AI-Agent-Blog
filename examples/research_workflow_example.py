...
async def progress_tracking_example():
    ...
    def track_progress(progress: WorkflowProgress):
        ...
        # Display progress
        print(f"\r[{bar}] {completion:.1f}% - {current_stage.upper()}", end="", flush=True)
...
async def strategy_comparison_example():
    ...
    print("-" * 50)
    print(f"{'Strategy':<15} {'Sources':<10} {'Findings':<10} {'Stats':<10} {'Time (s)':<10}")
    print("-" * 50)
    
    for strategy, metrics in results.items():
        print(f"{strategy:<15} {metrics['sources']:<10} {metrics['findings']:<10} "
...
async def adaptive_research_example():
    ...
    findings = await run_research_workflow(
        agent, keyword, config,
        progress_callback=track_adaptations
    )
...
class DetailedProgressTracker:
    ...
    def get_report(self):
        ...
        report = {
            ...
            "average_stage_time": sum(self.stage_durations.values()) / len(self.stage_durations) if self.stage_durations else 0
        }
