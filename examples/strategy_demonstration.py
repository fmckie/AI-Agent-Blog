...
from research_agent.strategy import ResearchStrategy, TopicType, ResearchDepth, ToolType
...
def demonstrate_tool_selection():
    ...
    test_cases = [
        ...
        (
            ...
            "Medical topic, exhaustive research",
        ),
...
def demonstrate_adaptive_strategy():
    ...
    for scenario in scenarios:
        ...
        print(f"\n   → Adaptations:")
        print(
            f"     Search queries: {len(initial_plan.search_queries)} → {len(adapted_plan.search_queries)}"
        )
        print(
            f"     Primary tools: {len(initial_plan.primary_tools)} → {len(adapted_plan.primary_tools)}"
        )
...
def demonstrate_edge_cases():
    ...
    for keyword, description in edge_cases:
        try:
            ...
            print(f"\n✅ {description}")
            print(
                f"   Keyword: '{keyword[:50]}...' (truncated)"
                if len(keyword) > 50
