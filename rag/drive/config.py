...
class DriveConfig(BaseSettings):
    ...
    auto_upload: bool = Field(
        default=True,
        description="Automatically upload articles after generation"
    )
    ...
    log_failures: bool = Field(
        default=True,
        description="Log upload failures to database"
    )
    notify_on_error: bool = Field(
        default=False,
        description="Send notifications on upload errors"
    )
    ...
    watch_folder_id: Optional[str] = Field(
        default=None,
        description="Google Drive folder ID to watch for new documents"
    )
    
    # Metadata settings
    include_keywords: bool = Field(
        default=True,
        description="Include keywords in document metadata"
    )
    include_sources: bool = Field(
        default=True,
        description="Include source URLs in document metadata"
    )
    include_generation_time: bool = Field(
        default=True,
        description="Include generation timestamp in metadata"
    )
    custom_properties: Dict[str, str] = Field(
        default={},
        description="Additional custom properties to add to uploaded files"
    )
