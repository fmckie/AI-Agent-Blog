...
class DriveStorageHandler:
    ...
    def track_upload(
        ...
        try:
            ...
            result = self.client.table("generated_articles").update({
    ...
    def get_sync_status(self, file_id: str) -> Optional[Dict[str, Any]]:
        ...
        """
        try:
            result = self.client.table("drive_sync_status").select(
                "*"
    ...
    def _update_sync_status(
        ...
        try:
            ...
            result = self.client.table("drive_sync_status").upsert(
                sync_data,
                on_conflict="file_id"
            ).execute()
    ...
    def get_pending_uploads(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            result = self.client.table("generated_articles").select(
                "id, title, keyword, file_path, created_at"
            ).is_("drive_file_id", "null").order(
    ...
    def get_drive_documents(
        ...
        """
        try:
            result = self.client.table("research_documents").select(
                "id, source, drive_file_id, created_at, metadata"
            ).eq("source_type", source_type).not_.is_(
                "drive_file_id", "null"
    ...
    def cleanup_orphaned_sync_records(self, dry_run: bool = True) -> Tuple[int, List[str]]:
        """
        ...
        try:
            ...
            sync_result = self.client.table("drive_sync_status").select("file_id, local_path").execute()
            ...
            for record in sync_records:
                ...
                if local_path.startswith("article_"):
                    article_id = local_path.replace("article_", "").replace("error_", "")
                    # Check if article exists
                    article_result = self.client.table("generated_articles").select(
                        "id"
                ...
                # Check if it's a document reference
                    ...
                    doc_result = self.client.table("research_documents").select(
                        "id"
