...
class ArticleUploader:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized ArticleUploader with folder ID: {self.upload_folder_id}"
        )
    ...
    def upload_file(
        ...
        if convert_to_docs and mime_type in [
            "text/html",
            "text/plain",
            "text/markdown",
        ]:
    ...
    def update_file_metadata(self, file_id: str, metadata: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            self.service.files().update(
                fileId=file_id, body={"properties": properties}
            ).execute()
    ...
    def _get_folder_tree(
        self, folder_id: str, level: int = 0, max_level: int = 3
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            # Build tree
            tree = {"id": folder["id"], "name": folder["name"], "folders": {}}

            # Recursively get subfolders
            for subfolder in subfolders:
                tree["folders"][subfolder["name"]] = self._get_folder_tree(
                    subfolder["id"], level + 1, max_level
                )
...
class BatchUploader(ArticleUploader):
                ...
                for article, future in futures:
                    try:
                        ...
                        if result["success"]:
                            ...
                            if (
                                "Missing file_path" not in error_msg
                                and "File not found" not in error_msg
                            ):
                                self.stats["failed"] += 1
                        ...
                        logger.error(
                            f"Upload failed for article {article.get('id')}: {e}"
                        )
                        self.stats["failed"] += 1
                        self.failed_uploads.append(article)
                        results.append(
    ...
    def _upload_article_with_retry(self, article: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                logger.error(
                    f"Upload attempt {attempt + 1} failed for {article_id}: {e}"
                )
    ...
    def retry_failed_uploads(
        ...
        return asyncio.run(
            self.upload_pending_articles(articles_to_retry, progress_callback)
        )
