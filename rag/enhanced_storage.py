...
class EnhancedVectorStorage(VectorStorage):
    ...
    async def store_research_source(
        ...
        try:
            ...
            logger.info(f"Stored research source: {source.title[:50]}... (ID: {source_id})")
        ...
        try:
            ...
            if reason:
                ...
                if current.data:
                    ...
                    metadata["credibility_updates"] = metadata.get("credibility_updates", [])
    ...
    async def get_crawl_hierarchy(self, root_url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for rel in relationships.data:
                ...
                child_hierarchy = await self.get_crawl_hierarchy(child_data.get("url", ""))
                if child_hierarchy:
    ...
    async def search_with_relationships(
        ...
        try:
            ...
            if not include_related:
                return [{"primary": chunk, "related": []} for chunk, _ in primary_results]
            
            # Get unique source IDs from results
            source_ids = list(set(
                chunk.get("source_id") for chunk, _ in primary_results 
                ...
                if source_id:
                    if source_id in combined_scores:
                        ...
                        combined_scores[source_id]["combined_score"] = (
                            weights["keyword"] * combined_scores[source_id]["keyword_score"] +
        ...
        try:
            ...
            for item in pending.data:
                ...
                try:
                    ...
                    logger.error(f"Failed to process embedding for source {source_data['id']}: {e}")
    ...
    async def _store_crawl_metadata(
        ...
        try:
            ...
            finding_data = {
                ...
                "main_findings": json.dumps([f"Found {len(source_ids)} relevant pages"]),
