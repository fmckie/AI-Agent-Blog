...
class ResearchStrategy:
    ...
    def __init__(self):
        ...
        self.topic_indicators = {
            TopicType.ACADEMIC: [
                "research", "study", "theory", "hypothesis", "methodology",
                "peer-reviewed", "journal", "academic", "scholar", "university",
                "dissertation", "thesis", "literature review", "meta-analysis"
            ],
            TopicType.TECHNICAL: [
                "programming", "coding", "software", "algorithm", "framework",
                "api", "documentation", "tutorial", "implementation", "debug",
                "architecture", "design pattern", "best practices", "optimization"
            ],
            TopicType.MEDICAL: [
                "health", "medical", "disease", "treatment", "clinical",
                "patient", "diagnosis", "therapy", "drug", "medication",
                "symptom", "condition", "healthcare", "epidemiology"
            ],
            TopicType.BUSINESS: [
                "business", "market", "finance", "economics", "strategy",
                "revenue", "profit", "investment", "startup", "enterprise",
                "management", "leadership", "marketing", "sales"
            ],
            TopicType.NEWS: [
                "latest", "recent", "breaking", "current", "today",
                "yesterday", "update", "announcement", "news", "trending",
                "happening", "development", "event"
            ],
            TopicType.EMERGING: [
                "new", "emerging", "cutting-edge", "breakthrough", "novel",
                "innovative", "future", "next-generation", "experimental",
                "beta", "preview", "upcoming"
            ]
        }
        
        # Domain preferences by topic type
        self.domain_preferences = {
            TopicType.ACADEMIC: [".edu", ".gov", "scholar.google.com", "pubmed.ncbi.nlm.nih.gov"],
            TopicType.TECHNICAL: [".io", "github.com", "stackoverflow.com", ".dev", "docs."],
    ...
    def analyze_topic(self, keyword: str, initial_context: Optional[str] = None) -> TopicType:
        """
        ...
        for topic_type, indicators in self.topic_indicators.items():
            ...
            keyword_score = sum(
                2 for indicator in indicators
                if indicator in keyword_lower
            )
            scores[topic_type] = score + keyword_score
        
        # Check for explicit emerging topics
        if any(indicator in keyword_lower for indicator in ["2024", "2025", "latest", "new"]):
    ...
    def select_tools(
        ...
        primary_tools.append(ToolRecommendation(
            ...
            parameters={"max_results": 10 if research_depth == ResearchDepth.SURFACE else 20}
        ))
        ...
        if research_depth in [ResearchDepth.DEEP, ResearchDepth.EXHAUSTIVE]:
            ...
            primary_tools.append(ToolRecommendation(
                ...
                parameters={"top_n_sources": 5 if research_depth == ResearchDepth.DEEP else 10}
            ))
        elif research_depth == ResearchDepth.STANDARD:
            # Standard research uses extraction as optional
            optional_tools.append(ToolRecommendation(
                tool=ToolType.EXTRACT,
                priority=7,
    ...
    def create_research_plan(
        ...
        """
        # Analyze the topic
        topic_type = self.analyze_topic(keyword, requirements.get("context") if requirements else None)
        ...
        target_domains = self._identify_target_domains(
            topic_type, available_domains
        )
    ...
    def adapt_strategy(
        ...
        if sources_found < 3 and source_quality < 0.7:
            ...
            current_plan.search_queries.extend([
                f"{current_plan.topic_type.value} {keyword}" for keyword in current_plan.search_queries[0].split()[:2]
            ])
            ...
            current_plan.primary_tools = [
                tool for tool in current_plan.primary_tools
                if tool.priority >= 8
            ]
    ...
    def _generate_search_queries(self, keyword: str, topic_type: TopicType) -> List[str]:
    ...
    def _generate_special_instructions(
        ...
        if topic_type == TopicType.ACADEMIC:
            instructions.append("Focus on peer-reviewed sources and recent publications")
            instructions.append("Extract methodology sections and statistical data")
        elif topic_type == TopicType.TECHNICAL:
            instructions.append("Prioritize official documentation and implementation examples")
