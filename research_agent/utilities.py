...
def calculate_source_diversity(sources: List[AcademicSource]) -> Dict[str, Any]:
    ...
    for source in sources:
        ...
        diversity_metrics["domain_distribution"][domain] = \
            diversity_metrics["domain_distribution"].get(domain, 0) + 1
    ...
    for source in sources:
        if source.publication_date:
            ...
            if year_match:
                ...
                diversity_metrics["year_distribution"][year_category] = \
                    diversity_metrics["year_distribution"].get(year_category, 0) + 1
    
    # Analyze credibility distribution
    for source in sources:
        if source.credibility_score >= 0.8:
            diversity_metrics["credibility_distribution"]["high"] += 1
        elif source.credibility_score >= 0.6:
            diversity_metrics["credibility_distribution"]["medium"] += 1
        else:
            diversity_metrics["credibility_distribution"]["low"] += 1
    
    # Calculate overall diversity score (0-1)
    if len(sources) > 0:
        # Domain diversity (more domains = higher score)
        domain_score = len(diversity_metrics["domain_distribution"]) / min(len(sources), 5)
        
        # Year diversity (mix of recent and established = higher score)
        year_score = len(diversity_metrics["year_distribution"]) / 3
        
        # Credibility balance (mostly high = good)
        cred_score = diversity_metrics["credibility_distribution"]["high"] / len(sources)
        
        # Combined score
        diversity_metrics["diversity_score"] = (domain_score + year_score + cred_score) / 3
...
def identify_conflicting_findings(findings: ResearchFindings) -> List[Dict[str, str]]:
    ...
    conflict_indicators = [
        "however", "contrary", "despite", "although", "whereas",
        "in contrast", "on the other hand", "conflicting", "disputed",
    ...
    if len(set(numbers)) > len(numbers) * 0.7:  # Many different numbers might indicate conflicts
...
def generate_research_questions(findings: ResearchFindings) -> List[str]:
    ...
    for gap in findings.research_gaps:
        ...
        if "further research" in gap.lower():
            question = gap.replace("further research is needed", "What research is needed")
            question = question.replace("further research needed", "What research is needed")
            questions.append(f"{question}?")
        elif "unclear" in gap.lower():
            questions.append(f"What factors contribute to the unclear aspects of {findings.keyword}?")
        elif "limited" in gap.lower():
            questions.append(f"How can we address the limited data on {findings.keyword}?")
    
    # Based on findings
    if findings.main_findings:
        questions.append(f"What are the long-term implications of {findings.main_findings[0]}?")
        questions.append(f"How can practitioners apply {findings.keyword} research in real-world settings?")
...
def assess_research_quality(findings: ResearchFindings) -> Dict[str, Any]:
    ...
    if findings.academic_sources:
        avg_credibility = sum(s.credibility_score for s in findings.academic_sources) / len(findings.academic_sources)
        quality_assessment["metrics"]["source_quality"] = avg_credibility
        
        if avg_credibility >= 0.8:
            quality_assessment["strengths"].append("High-quality academic sources")
        elif avg_credibility < 0.6:
            quality_assessment["weaknesses"].append("Low average source credibility")
            quality_assessment["recommendations"].append("Seek more peer-reviewed sources")
    
    # Assess source quantity
    source_count = len(findings.academic_sources)
    if source_count >= 5:
        quality_assessment["metrics"]["source_quantity"] = 1.0
        quality_assessment["strengths"].append(f"Comprehensive coverage with {source_count} sources")
    elif source_count >= 3:
        quality_assessment["metrics"]["source_quantity"] = 0.7
    else:
        quality_assessment["metrics"]["source_quantity"] = 0.4
        quality_assessment["weaknesses"].append("Limited number of sources")
        quality_assessment["recommendations"].append("Expand search to find more sources")
    
    # Assess content depth
    if findings.key_statistics and len(findings.key_statistics) >= 3:
        quality_assessment["metrics"]["content_depth"] = 0.8
        quality_assessment["strengths"].append("Rich statistical evidence")
    else:
        quality_assessment["metrics"]["content_depth"] = 0.5
        quality_assessment["recommendations"].append("Extract more quantitative data")
    ...
    recency_ratio = recent_sources / len(findings.academic_sources) if findings.academic_sources else 0
    quality_assessment["metrics"]["recency"] = recency_ratio
    
    if recency_ratio >= 0.7:
        quality_assessment["strengths"].append("Mostly recent sources")
    elif recency_ratio < 0.3:
        quality_assessment["weaknesses"].append("Many outdated sources")
        quality_assessment["recommendations"].append("Prioritize more recent research")
    
    # Get diversity score
    diversity = calculate_source_diversity(findings.academic_sources)
    quality_assessment["metrics"]["diversity"] = diversity["diversity_score"]
    ...
    if quality_assessment["overall_score"] < 0.6:
        quality_assessment["recommendations"].insert(0, "Consider conducting additional research")
    elif quality_assessment["overall_score"] > 0.8:
        quality_assessment["strengths"].insert(0, "Excellent research quality overall")
