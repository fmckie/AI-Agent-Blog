...
class ResearchWorkflow:
    ...
    async def execute_research_pipeline(
        ...
        """
        logger.info(f"Starting research workflow for '{keyword}' with strategy: {strategy}")
        ...
        for stage in stages:
            try:
                ...
                if self.config.enable_adaptive_strategy and stage == WorkflowStage.DISCOVERY:
        ...
        return context.get("findings", ResearchFindings(
            keyword=keyword,
    ...
    async def _handle_crawling(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        edu_gov_sources = [
            s for s in sources 
            if '.edu' in s.url or '.gov' in s.url
        ]
    ...
    def _adapt_strategy_based_on_results(self, context: Dict[str, Any]):
        ...
        avg_credibility = sum(s.credibility_score for s in sources) / sources_count if sources_count > 0 else 0
