...
def check_dependencies() -> dict[str, bool]:
    ...
    try:
        subprocess.run(
            ["pytest", "--version", "--timeout=1"], capture_output=True, check=True
        )
...
def main():
    ...
    if not deps["pytest-xdist"] and args.parallel:
        print_status(
            "pytest-xdist not installed, parallel execution disabled", "warning"
        )
    ...
    if args.full:
        ...
        try:
            run_command(
                ["isort", ".", "--check-only", "--quiet"], "Import sorting (isort)"
            )
        except FileNotFoundError:
    ...
    if total_time > 300:  # More than 5 minutes
        ...
        print_status(
            "  • Use --unit-only for quick feedback during development", "info"
        )
