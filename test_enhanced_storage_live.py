...
class EnhancedStorageTester:
        ...
        if self.test_source_ids:
            try:
                for source_id in self.test_source_ids:
                    self.storage.supabase.table("research_sources").delete().eq(
                        "id", source_id
                    ).execute()
                print(
        ...
        for source in sources:
            try:
                source_id = await self.storage.store_research_source(
                    source,
                    full_content=f"Full content of {source.title}. "
                    * 100,  # Simulate full content
        ...
        if stored_ids:
            try:
                ...
                success = (
                    ...
                    and retrieved["title"] == sources[0].title
                    and retrieved["credibility_score"] == sources[0].credibility_score
                )
    ...
    async def test_source_relationships(self, source_ids: List[str]):
        ...
        if len(source_ids) < 2:
            print(
                f"{Fore.YELLOW}Skipping relationship tests - need at least 2 sources{Style.RESET_ALL}"
            )
            return
        ...
        try:
            ...
            for rel in related[:3]:  # Show first 3
                print(
                    f"  - {rel['relationship_type']}: {rel['source'].get('title', 'Unknown')[:50]}..."
                )
    ...
    async def test_crawl_storage(self):
        ...
        try:
            stored_ids = await self.storage.store_crawl_results(
                crawl_data, parent_url, "AI research methodology"
            )
            self.test_source_ids.extend(stored_ids)  # Track for cleanup

            self.print_test(
                f"Store crawl results ({len(stored_ids)} pages)", len(stored_ids) == 3
            )
        ...
        try:
            ...
            has_hierarchy = (
                hierarchy.get("url") == parent_url and "children" in hierarchy
            )
    ...
    async def test_advanced_search(self):
        ...
        try:
            results = await self.storage.search_by_criteria(
                domain=".edu", min_credibility=0.8, limit=5
            )
            self.print_test(f"Search by criteria (found {len(results)})", True)

            for result in results[:3]:
                print(
                    f"  - {result['title'][:50]}... (score: {result['credibility_score']})"
                )
        except Exception as e:
        ...
        try:
            results = await self.storage.search_by_criteria(keyword="learning", limit=5)
        ...
        try:
            ...
            for i, result in enumerate(results[:3]):
                ...
                print(
                    f"     - Keyword: {result['keyword_score']:.1f}, Vector: {result['vector_score']:.3f}"
                )
        except Exception as e:
    ...
    async def test_batch_operations(self):
        ...
        try:
            ...
            stored_ids = await self.storage.batch_store_sources(
                batch_sources, generate_embeddings=True  # Queue for embedding
            )
        ...
        try:
            ...
            self.print_test(
                f"Process embedding queue (processed {processed})", processed >= 0
            )

            # Check queue status
            queue_status = (
                self.storage.supabase.table("embedding_queue")
                .select("status")
                .execute()
            )
            status_counts = {}
