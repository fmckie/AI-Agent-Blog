...
class ManualStorageTester:
    ...
    async def test_1_basic_storage(self):
        ...
        test_source = {
            ...
            "full_content": "This is a comprehensive article about machine learning. " * 50,
        ...
        try:
            # Insert source
            result = self.supabase.table("research_sources").insert(test_source).execute()
            ...
            # Retrieve source
            result = self.supabase.table("research_sources").select("*").eq("id", source_id).execute()
            ...
            result = self.supabase.table("research_sources").update(update_data).eq("id", source_id).execute()
    ...
    async def test_2_chunks_with_embeddings(self, source_id: str = None):
        ...
        if not source_id:
            ...
            result = self.supabase.table("research_sources").insert(source_data).execute()
            source_id = result.data[0]["id"]
            self.test_source_ids.append(source_id)
        ...
        try:
            ...
            self.print_test(f"Insert {len(chunks)} chunks with embeddings", len(result.data) == len(chunks))
    ...
    async def test_3_relationships(self):
        ...
        try:
            ...
            for i in range(2):
                ...
                result = self.supabase.table("research_sources").insert(source).execute()
            ...
            result = self.supabase.table("source_relationships").insert(relationship).execute()
            self.print_test("Create source relationship", len(result.data) > 0)
            
            # Query relationships
            result = self.supabase.table("source_relationships").select(
                "*, research_sources!related_source_id(title, url)"
            ).eq("source_id", sources[0]).execute()
            
            self.print_test(
                f"Query relationships (found {len(result.data)})", 
                len(result.data) > 0
            )
    ...
    async def test_4_crawl_storage(self):
        ...
        try:
            ...
            result = self.supabase.table("research_sources").insert(parent_source).execute()
            ...
            result = self.supabase.table("source_relationships").select(
                "*, research_sources!source_id(title, url)"
            ).eq("related_source_id", parent_id).eq("relationship_type", "crawled_from").execute()
    ...
    async def test_5_advanced_search(self):
        ...
        try:
            ...
            result = self.supabase.table("research_sources").select("*").eq(
                "domain", ".edu"
            ).gte("credibility_score", 0.7).order(
                "credibility_score", desc=True
            ).limit(5).execute()
            
            self.print_test(
                f"Multi-criteria search (found {len(result.data)})", 
                True
            )
            
            if result.data:
                for source in result.data[:3]:
                    print(f"  - {source['title'][:40]}... (score: {source['credibility_score']})")
            
            # Keyword search
            keyword = "learning"
            result = self.supabase.table("research_sources").select("*").or_(
                f"title.ilike.%{keyword}%,excerpt.ilike.%{keyword}%"
            ).limit(5).execute()
            
            self.print_test(
                f"Keyword search for '{keyword}' (found {len(result.data)})", 
                True
            )
    ...
    async def test_6_performance(self):
        ...
        try:
            ...
            result = self.supabase.table("research_sources").insert(batch_sources).execute()
            ...
            print(f"  - Time: {elapsed:.2f}s ({elapsed/len(batch_sources):.3f}s per source)")
            ...
            result = self.supabase.table("embedding_queue").insert(queue_items).execute()
            self.print_test(
        ...
        try:
            ...
            if self.test_source_ids:
                ...
                for i in range(0, len(self.test_source_ids), batch_size):
                    ...
                    self.supabase.table("research_sources").delete().in_("id", batch).execute()
                
                print(f"{Fore.GREEN}✓ Cleaned up {len(self.test_source_ids)} test sources{Style.RESET_ALL}")
            
            # Clean up embedding queue
            self.supabase.table("embedding_queue").delete().eq("status", "pending").execute()
...
async def main():
    """Main entry point."""
    print(f"{Fore.YELLOW}Starting manual test of Enhanced Storage...{Style.RESET_ALL}\n")
