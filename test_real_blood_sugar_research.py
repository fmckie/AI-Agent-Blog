...
async def test_real_blood_sugar_research():
    ...
    try:
        ...
        if config.tavily_api_key.startswith("tvly-test") or config.openai_api_key.startswith("sk-test"):
            logger.error("❌ Error: Still using test API keys. Please set real keys in .env file")
            return
        ...
        logger.info("⏳ This may take 10-30 seconds as we search real academic databases...\n")
        ...
        logger.info(f"📊 Total Sources Analyzed: {research_findings.total_sources_analyzed}")
