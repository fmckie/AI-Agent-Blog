...
async def test_research_with_caching():
    ...
    stats_table.add_row("Semantic Cache Hits", str(retriever_stats.get("semantic_hits", 0)))
    ...
    async with VectorStorage() as storage:
        ...
        storage_table.add_row("Research Chunks", str(storage_stats.get("total_chunks", 0)))
        storage_table.add_row("Cache Entries", str(storage_stats.get("total_cache_entries", 0)))
        storage_table.add_row("Unique Keywords", str(storage_stats.get("unique_keywords", 0)))
    ...
    if duration2 < duration:
        ...
        console.print(f"[bold green]⚡ {speedup:.1f}% faster due to caching![/bold green]")
    ...
    if final_retriever_stats.get("exact_hits", 0) > 0:
        console.print("[green]✓ Cache hit confirmed! Research was served from cache.[/green]")
    else:
        console.print("[yellow]⚠ No cache hit detected. Check cache configuration.[/yellow]")
