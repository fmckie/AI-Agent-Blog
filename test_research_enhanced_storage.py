...
async def test_research_with_storage():
    ...
    try:
        ...
        if not config.openai_api_key:
            print(
                f"{Fore.RED}❌ OpenAI API key not found in environment{Style.RESET_ALL}"
            )
            return
        if not config.tavily_api_key:
            print(
                f"{Fore.RED}❌ Tavily API key not found in environment{Style.RESET_ALL}"
            )
            return
        ...
        try:
            ...
            print(
                f"{Fore.GREEN}✓ EnhancedVectorStorage initialized successfully{Style.RESET_ALL}"
            )

            # Test connection
            test_source = await storage.get_source_by_url(
                "https://test.nonexistent.url"
            )
        ...
        if storage:
            ...
            for source in research_findings.academic_sources:
                ...
                if source_data:
                    stored_count += 1
                    print(
                        f"{Fore.GREEN}✓ Found in storage: {source.title[:50]}...{Style.RESET_ALL}"
                    )
            ...
            edu_sources = await storage.search_by_criteria(
                domain=".edu", min_credibility=0.7, keyword="diet", limit=5
            )
        ...
        output_file = (
            f"test_research_enhanced_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
        ...
        if storage:
            ...
            print(
                f"{Fore.GREEN}✓ Source Storage: {stored_count} sources stored{Style.RESET_ALL}"
            )
        else:
            print(
                f"{Fore.YELLOW}⚠️ EnhancedVectorStorage: Not available{Style.RESET_ALL}"
            )
