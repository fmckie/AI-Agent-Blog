...
async def test_research_workflow():
    ...
    try:
        ...
        if storage:
            ...
            print(
                f"{Fore.GREEN}✓ Stored {stored_count}/{len(research_findings.academic_sources)} sources{Style.RESET_ALL}"
            )
            print(
                f"{Fore.GREEN}✓ Created {relationship_count} relationships{Style.RESET_ALL}"
            )
            ...
            diet_sources = await storage.search_by_criteria(
                keyword="diet", min_credibility=0.5, limit=20
            )
        ...
        print(
            f"{Fore.GREEN}✓ Sources found: {len(research_findings.academic_sources)}{Style.RESET_ALL}"
        )
        if storage:
