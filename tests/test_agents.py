...
class TestResearchAgent:
    ...
    async def test_research_agent_execution_success(
        self, test_config, mock_tavily_response
    ):
        """
        ...
        mock_result = Mock()
        mock_result.data = expected_findings
    ...
    async def test_research_agent_no_sources_found(self, test_config):
        ...
        with patch.object(
            agent, "run", side_effect=ValueError("No academic sources found")
        ):
    ...
    async def test_research_agent_with_mock_tools(self, test_config):
        ...
        # Mock tool functions
        with patch(
            "research_agent.tools.search_academic", return_value=mock_search_response
        ):
    ...
    async def test_research_agent_retry_on_failure(self, test_config):
        ...
        async def failing_research():
            ...
            return ResearchFindings(
                keyword="test",
                research_summary="Success after retries - This test demonstrates that the retry mechanism is functioning correctly and can handle temporary failures gracefully.",
                academic_sources=[
                    AcademicSource(
                        title="Test Source",
                        url="https://test.edu",
...
def test_statistics_extraction():
    ...
    assert "23%" in stats
    assert "89%" in stats  # The function converts "89 percent" to "89%"
