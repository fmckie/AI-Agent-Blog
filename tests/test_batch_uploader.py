...
class TestBatchUploaderProcessing:
    ...
    def mock_uploader(self):
        ...
        with patch("rag.drive.config.get_drive_config") as mock_get_config:
            ...
            uploader.upload_html_as_doc = Mock(
                return_value={
    ...
    @pytest.mark.asyncio
    async def test_upload_pending_articles_single_success(
        self, mock_uploader, tmp_path
    ):
        ...
        mock_uploader._upload_article_with_retry = Mock(
            return_value={
                "article_id": "123",
                "success": True,
    ...
    async def test_upload_pending_with_progress_callback(self, mock_uploader, tmp_path):
        ...
        assert (
            "Test Article with Very Long Title That Should Be T..."
            in progress_calls[0]["message"]
        )
...
class TestBatchUploaderRetryLogic:
    ...
    def test_upload_article_with_retry_success_first_try(
        self, uploader_with_retry, tmp_path
    ):
    ...
    def test_upload_article_with_retry_success_after_retry(
        self, uploader_with_retry, tmp_path
    ):
