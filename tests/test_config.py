...
class TestConfig:
    ...
    def test_inline_comment_stripping(self, monkeypatch):
        """
        Test that inline comments in .env files are properly stripped.

        This ensures that values like "advanced  # Options: basic, advanced"
        are cleaned to just "advanced".
        """
        # Set up environment with inline comments
        monkeypatch.setenv(
            "TAVILY_API_KEY", "test_tavily_key_that_is_long_enough  # This is a comment"
        )
        monkeypatch.setenv(
            "OPENAI_API_KEY", "test_openai_key_that_is_long_enough # Another comment"
        )
        monkeypatch.setenv("LLM_MODEL", "gpt-4  # Model choice")
        monkeypatch.setenv(
            "TAVILY_SEARCH_DEPTH", "advanced  # Options: basic, advanced"
        )
        monkeypatch.setenv(
            "TAVILY_MAX_RESULTS", "10  # Number of search results (1-20)"
        )
        monkeypatch.setenv("LOG_LEVEL", "INFO  # Logging level")
