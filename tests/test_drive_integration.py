    ...
    async def test_workflow_with_drive_upload_enabled(
        ...
        with patch('rag.config.get_rag_config') as mock_rag_config:
            ...
            with patch('workflow.GoogleDriveAuth') as mock_auth_class, \
                ...
                # Mock the agent runs
                with patch.object(orchestrator, 'run_research', return_value=mock_research), \
                     patch.object(orchestrator, 'run_writing', return_value=mock_article):
                    ...
                    assert orchestrator.workflow_data["drive_upload"]["file_id"] == "drive_file_123"
...
class TestDriveStorageHandler:
    ...
    def test_track_upload(self):
        ...
        with patch('rag.drive.storage.create_client') as mock_create_client:
            ...
            mock_table.update.return_value.eq.return_value.execute.return_value.data = [{"id": "123"}]
