...
class TestArticleUploader:
    ...
    def test_get_upload_folder_structure(self, uploader):
        ...
        # Mock folder tree
        uploader._get_folder_tree = Mock(
            return_value={
                "id": "root_id",
                "name": "Uploads",
                "folders": {"2025": {"id": "2025_id", "name": "2025", "folders": {}}},
            }
        )
...
class TestBatchUploader:
    ...
    def batch_uploader(self, mock_auth, mock_drive_config):
        ...
        with patch(
            "rag.drive.uploader.get_drive_config", return_value=mock_drive_config
        ):
    ...
    async def test_upload_pending_with_failures(self, batch_uploader, tmp_path):
        ...
        assert (
            len(batch_uploader.failed_uploads) == 0
        )  # Skipped articles not added to failed
