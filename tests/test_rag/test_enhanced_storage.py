...
from models import (
    AcademicSource, ExtractedContent, CrawledPage,
    StoredSource, SourceRelationship, SearchResult
)
...
class TestResearchSourceManagement:
    ...
    @pytest.mark.asyncio
    async def test_store_research_source_basic(self, storage, mock_supabase, sample_academic_source):
        ...
        assert call_args["credibility_score"] == sample_academic_source.credibility_score
    
    @pytest.mark.asyncio
    async def test_store_research_source_with_content(self, storage, mock_supabase, sample_academic_source):
        ...
        mock_supabase.table().insert().execute.return_value = Mock(data=[{"id": "queue-id"}])
        
        # Mock chunk processing
        with patch.object(storage, '_process_and_store_chunks', return_value=["chunk1", "chunk2"]):
...
class TestCrawlResultStorage:
    ...
    async def test_get_crawl_hierarchy(self, storage, mock_supabase):
        ...
        # Mock root source
        with patch.object(storage, 'get_source_by_url', return_value={
            "id": root_id,
            "title": "Root Page",
            "url": root_url
        }):
...
class TestAdvancedSearchMethods:
    ...
    async def test_search_by_criteria(self, storage, mock_supabase):
        ...
        # Setup mock response
        mock_supabase.table().select().gte().order().limit().execute.return_value = Mock(
            data=[
                {
                    "id": "source1",
    ...
    async def test_search_with_relationships(self, storage, mock_supabase):
        ...
        with patch.object(storage, 'search_similar_chunks', return_value=[
            ({"source_id": "source1", "content": "Content 1"}, 0.9),
            ({"source_id": "source2", "content": "Content 2"}, 0.85)
        ]):
            # Mock related sources
            with patch.object(storage, 'get_related_sources', return_value=[
                {
    ...
    async def test_hybrid_search(self, storage, mock_supabase):
        ...
        # Mock keyword search
        with patch.object(storage, 'search_by_criteria', return_value=[
            {"id": "source1", "title": "Keyword Match"}
        ]):
            # Mock vector search
            with patch.object(storage, 'search_similar_chunks', return_value=[
...
class TestBatchOperations:
    ...
    @pytest.mark.asyncio
    async def test_batch_store_sources(self, storage, mock_supabase, sample_academic_source):
