...
class TestDriveStorageHandler:
    ...
    def mock_config(self):
        ...
        with (
            patch("rag.drive.storage.get_config") as mock_get_config,
    ...
    def storage_handler(self, mock_config, mock_supabase_client):
        ...
        with patch(
            "rag.drive.storage.create_client", return_value=mock_supabase_client
        ):
    ...
    def test_init_without_client(self, mock_config):
        ...
        with patch(
            "rag.drive.storage.create_client", return_value=mock_client
        ) as mock_create:
    ...
    def test_track_drive_document_no_metadata(
        self, storage_handler, mock_supabase_client
    ):
    ...
    def test_get_uploaded_articles_success(self, storage_handler, mock_supabase_client):
        ...
        mock_supabase_client.table.assert_called_with("generated_articles")
        mock_supabase_client.table.return_value.not_.is_.assert_called_with(
            "drive_file_id", "null"
        )

    def test_get_uploaded_articles_with_metadata(
        self, storage_handler, mock_supabase_client
    ):
    ...
    def test_get_pending_uploads_success(self, storage_handler, mock_supabase_client):
        ...
        mock_supabase_client.table.return_value.is_.assert_called_with(
            "drive_file_id", "null"
        )
    ...
    def test_mark_upload_error_success(self, storage_handler):
        ...
        with patch.object(
            storage_handler, "_update_sync_status", return_value=True
        ) as mock_update:
    ...
    def test_get_drive_documents_invalid_metadata(
        self, storage_handler, mock_supabase_client
    ):
        """Test handling invalid JSON metadata."""
        # Mock document with invalid metadata
        docs = [
            {"id": str(uuid4()), "source": "broken.pdf", "metadata": "invalid json"}
        ]
    ...
    def test_cleanup_orphaned_sync_records_dry_run(
        self, storage_handler, mock_supabase_client
    ):
        ...
        # Call method
        count, orphaned_ids = storage_handler.cleanup_orphaned_sync_records(
            dry_run=True
        )
    ...
    def test_cleanup_orphaned_sync_records_actual_delete(
        self, storage_handler, mock_supabase_client
    ):
        """Test cleanup with actual deletion."""
        # Mock sync records
        sync_records = [{"file_id": "orphan-1", "local_path": "article_missing"}]
    ...
    def test_exception_handling_in_all_methods(
        self, storage_handler, mock_supabase_client
    ):
        ...
        assert (
            storage_handler.track_drive_document("id", "name", "type", "url", "path")
            is None
        )
        assert storage_handler.get_uploaded_articles() == []
        assert storage_handler.get_sync_status("id") is None
        assert (
            storage_handler._update_sync_status("id", "path", "url", "status") is False
        )
