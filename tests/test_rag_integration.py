...
class TestRAGIntegration:
    ...
    @pytest.mark.asyncio
    async def test_full_rag_pipeline_new_research(self, mock_configs, sample_research_data):
                ...
                with patch("rag.embeddings.get_config", return_value=main_config):
                    with patch("rag.embeddings.get_rag_config", return_value=rag_config):
                        with patch("rag.storage.create_client") as mock_create_client:
                            ...
                            # Mock chunk storage
                            mock_table.upsert = Mock(return_value=Mock(
                                execute=Mock(return_value=Mock(
                                    data=[{"id": f"chunk_{i}"} for i in range(10)]
                                ))
                            ))
                            ...
                            with patch("rag.embeddings.AsyncOpenAI") as mock_openai_class:
    ...
    @pytest.mark.asyncio
    async def test_full_rag_pipeline_with_cache_hit(self, mock_configs, sample_research_data):
                ...
                with patch("rag.embeddings.get_config", return_value=main_config):
                    with patch("rag.embeddings.get_rag_config", return_value=rag_config):
                        with patch("rag.storage.create_client") as mock_create_client:
                            ...
                            mock_select1 = Mock()
                            mock_eq1 = Mock(return_value=Mock(
                            ...
                            mock_update = Mock()
                            mock_eq2 = Mock(return_value=Mock(
                                execute=Mock(return_value=Mock(data=[]))
                            ))
                            ...
                            mock_select2 = Mock()
                            mock_in = Mock(return_value=Mock(
                                execute=Mock(return_value=Mock(data=[]))
                            ))
                            mock_select2.in_ = mock_in
                            
                            mock_table.select = Mock(side_effect=[mock_select1, mock_select2])
                            ...
                            with patch("rag.storage.asyncpg.create_pool", AsyncMock()):
                                ...
                                assert result.keyword == "AI healthcare applications"
                                assert result.research_summary == sample_research_data.research_summary
    ...
    @pytest.mark.asyncio
    async def test_rag_semantic_search_integration(self, mock_configs, sample_research_data):
            ...
            with patch("rag.config.get_rag_config", return_value=rag_config):
                with patch("rag.embeddings.get_config", return_value=main_config):
                    with patch("rag.embeddings.get_rag_config", return_value=rag_config):
                        ...
                        with patch.object(embeddings, "_generate_single_embedding") as mock_gen:
                        ...
                        with patch("rag.storage.create_client"):
                            with patch("rag.embeddings.AsyncOpenAI"):
                                with patch("rag.storage.asyncpg.create_pool", AsyncMock()):
                                    ...
                                    assert float(retriever_stats["cache_hit_rate"].rstrip("%")) == 60.0
                                    
                                    # Verify embedding metrics
                                    assert embedding_stats["cache_hit_rate"] == "75.0%"
                                    assert embedding_stats["total_tokens"] == 10000
                                    assert float(embedding_stats["total_cost_usd"].lstrip("$")) == 0.0002
        ...
        with patch("config.get_config", return_value=main_config):
            with patch("rag.config.get_rag_config", return_value=rag_config):
                with patch("rag.storage.create_client") as mock_create_client:
                    ...
                    mock_create_client.side_effect = Exception("Database connection failed")
    ...
    @pytest.mark.asyncio
    async def test_rag_cache_warming_integration(self, mock_configs, sample_research_data):
                ...
                with patch("rag.embeddings.get_config", return_value=main_config):
                    with patch("rag.embeddings.get_rag_config", return_value=rag_config):
                        with patch("rag.storage.create_client") as mock_create_client:
                            ...
                            mock_table.upsert = Mock(return_value=Mock(
                                execute=Mock(return_value=Mock(
                                    data=[{"id": f"chunk_{i}"} for i in range(5)]
                                ))
                            ))
                            ...
                            with patch("rag.embeddings.AsyncOpenAI") as mock_openai_class:
                                ...
                                with patch("rag.storage.asyncpg.create_pool", AsyncMock()):
                                    ...
                                    # Warm cache
                                    results = await retriever.warm_cache(keywords, mock_research)
    ...
    @pytest.mark.asyncio
    async def test_text_processing_integration(self, mock_configs, sample_research_data):
        ...
        with patch("rag.config.get_rag_config", return_value=rag_config):
            ...
            for chunk in chunks:
                ...
                assert len(chunk.content) <= rag_config.chunk_size + rag_config.chunk_overlap
            
            # Verify all content types are represented
            source_types = {chunk.metadata["source_type"] for chunk in chunks}
            expected_types = {"research_summary", "academic_source", "main_findings", "statistics"}
