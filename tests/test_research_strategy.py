...
class TestTopicClassification:
    ...
    def test_academic_topic_classification(self, strategy):
        ...
        assert (
            strategy.analyze_topic("quantum computing research") == TopicType.ACADEMIC
        )
        assert (
            strategy.analyze_topic("peer-reviewed study on climate change")
            == TopicType.ACADEMIC
        )
        assert (
            strategy.analyze_topic("meta-analysis of educational methods")
            == TopicType.ACADEMIC
        )
        assert (
            strategy.analyze_topic("dissertation on machine learning")
            == TopicType.ACADEMIC
        )
        ...
        assert (
            strategy.analyze_topic(
                "AI ethics", "looking for academic papers and research studies"
            )
            == TopicType.ACADEMIC
        )
    ...
    def test_technical_topic_classification(self, strategy):
        ...
        assert (
            strategy.analyze_topic("Python programming tutorial") == TopicType.TECHNICAL
        )
        assert (
            strategy.analyze_topic("API documentation for REST services")
            == TopicType.TECHNICAL
        )
        assert (
            strategy.analyze_topic("debugging JavaScript frameworks")
            == TopicType.TECHNICAL
        )
        assert (
            strategy.analyze_topic("software architecture patterns")
            == TopicType.TECHNICAL
        )
        assert (
            strategy.analyze_topic("algorithm optimization techniques")
            == TopicType.TECHNICAL
        )
    ...
    def test_medical_topic_classification(self, strategy):
        ...
        assert (
            strategy.analyze_topic("clinical trials for cancer therapy")
            == TopicType.MEDICAL
        )
        assert (
            strategy.analyze_topic("patient outcomes in healthcare")
            == TopicType.MEDICAL
        )
    ...
    def test_business_topic_classification(self, strategy):
        ...
        assert (
            strategy.analyze_topic("startup investment strategies")
            == TopicType.BUSINESS
        )
        assert (
            strategy.analyze_topic("market analysis for e-commerce")
            == TopicType.BUSINESS
        )
        assert (
            strategy.analyze_topic("revenue optimization techniques")
            == TopicType.BUSINESS
        )
        assert (
            strategy.analyze_topic("business leadership principles")
            == TopicType.BUSINESS
        )
    ...
    def test_news_topic_classification(self, strategy):
        ...
        assert (
            strategy.analyze_topic("recent updates in quantum computing")
            == TopicType.NEWS
        )
    ...
    def test_emerging_topic_classification(self, strategy):
        ...
        assert (
            strategy.analyze_topic("new breakthrough in quantum computing")
            == TopicType.EMERGING
        )
        assert (
            strategy.analyze_topic("cutting-edge AI research 2024")
            == TopicType.EMERGING
        )
        assert strategy.analyze_topic("novel coronavirus variant") == TopicType.EMERGING
        assert (
            strategy.analyze_topic("next-generation battery technology")
            == TopicType.EMERGING
        )
    ...
    def test_mixed_indicator_classification(self, strategy):
        ...
        assert (
            strategy.analyze_topic("clinical research on new treatment methods")
            == TopicType.MEDICAL
        )
...
class TestResearchDepthDetermination:
    ...
    def test_quick_research_depth(self, strategy):
        ...
        depth = strategy.determine_research_depth("AI basics", {"quick": True})
        assert depth == ResearchDepth.SURFACE

        depth = strategy.determine_research_depth("Python overview", {"overview": True})
        assert depth == ResearchDepth.SURFACE
    ...
    def test_comprehensive_research_depth(self, strategy):
        ...
        depth = strategy.determine_research_depth(
            "quantum computing", {"comprehensive": True}
        )
        assert depth == ResearchDepth.DEEP

        depth = strategy.determine_research_depth(
            "machine learning", {"thorough": True}
        )
        assert depth == ResearchDepth.DEEP
    ...
    def test_exhaustive_research_depth(self, strategy):
        ...
        depth = strategy.determine_research_depth(
            "climate change", {"exhaustive": True}
        )
        assert depth == ResearchDepth.EXHAUSTIVE

        depth = strategy.determine_research_depth(
            "medical research", {"complete": True}
        )
        assert depth == ResearchDepth.EXHAUSTIVE
    ...
    def test_time_constraint_depth(self, strategy):
        ...
        depth = strategy.determine_research_depth(
            "test topic", {"time_limit_minutes": 3}
        )
        assert depth == ResearchDepth.SURFACE

        # Long time = deep
        depth = strategy.determine_research_depth(
            "test topic", {"time_limit_minutes": 45}
        )
        assert depth == ResearchDepth.DEEP

        # Medium time = standard (default)
        depth = strategy.determine_research_depth(
            "test topic", {"time_limit_minutes": 15}
        )
        assert depth == ResearchDepth.STANDARD
    ...
    def test_complex_topic_depth_upgrade(self, strategy):
        ...
        surface_complex = strategy.determine_research_depth(
            "quantum computing vs classical computing performance", {"quick": True}
        )
...
class TestToolSelection:
    ...
    def test_deep_research_tool_selection(self, strategy):
        ...
        primary, optional = strategy.select_tools(
            TopicType.ACADEMIC, ResearchDepth.DEEP, []
        )
    ...
    def test_academic_topic_tool_selection(self, strategy):
        ...
        primary, optional = strategy.select_tools(
            TopicType.ACADEMIC, ResearchDepth.STANDARD, []
        )
    ...
    def test_medical_topic_tool_selection(self, strategy):
        ...
        primary, optional = strategy.select_tools(
            TopicType.MEDICAL, ResearchDepth.STANDARD, []
        )
    ...
    def test_emerging_topic_tool_selection(self, strategy):
        ...
        primary, optional = strategy.select_tools(
            TopicType.EMERGING, ResearchDepth.STANDARD, []
        )
    ...
    def test_exhaustive_research_tool_selection(self, strategy):
        ...
        primary, optional = strategy.select_tools(
            TopicType.ACADEMIC, ResearchDepth.EXHAUSTIVE, []
        )
...
class TestResearchPlanCreation:
    ...
    def test_research_plan_with_initial_results(self, strategy):
        ...
        plan = strategy.create_research_plan(
            "machine learning optimization", initial_results=initial_results
        )
    ...
    def test_domain_targeting(self, strategy):
        ...
        assert any(
            "github.com" in d or ".io" in d for d in technical_plan.target_domains
        )

        # Medical should include health organizations
        medical_plan = strategy.create_research_plan("COVID-19 vaccines")
        assert any(
            "nih.gov" in d or "who.int" in d for d in medical_plan.target_domains
        )
    ...
    def test_special_instructions_generation(self, strategy):
        ...
        academic_plan = strategy.create_research_plan(
            "climate change research", {"research_depth": "exhaustive"}
        )
...
class TestAdaptiveStrategy:
    ...
    def test_adapt_strategy_excellent_results(self, strategy, initial_plan):
        ...
        extract_tool = next(
            (t for t in adapted_plan.primary_tools if t.tool == ToolType.EXTRACT), None
        )
        if extract_tool:
    ...
    def test_adapt_strategy_academic_domains(self, strategy):
        ...
        initial_plan = strategy.create_research_plan(
            "quantum physics research", {"strategy": "standard"}
        )
