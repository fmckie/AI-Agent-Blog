...
class TestResearchWorkflow:
    ...
    async def test_workflow_initialization(self, mock_agent, mock_config):
        ...
        workflow = ResearchWorkflow(
            agent=mock_agent, config=mock_config, progress_callback=progress_callback
        )
    ...
    async def test_handler_validation(self, mock_agent, mock_config, sample_findings):
        ...
        )  # -20 for sources, -20 for findings, -20 for stats
    ...
    @pytest.mark.asyncio
    async def test_execute_research_pipeline_basic(
        self, mock_agent, mock_config, sample_findings
    ):
        ...
        findings = await workflow.execute_research_pipeline(
            keyword="test keyword", strategy="basic", max_retries=1
        )
    ...
    @pytest.mark.asyncio
    async def test_execute_research_pipeline_with_failure(
        ...
        with pytest.raises(WorkflowError, match="Critical stage discovery failed"):
            await workflow.execute_research_pipeline(
                keyword="test", strategy="basic", max_retries=1
            )

    @pytest.mark.asyncio
    async def test_execute_research_pipeline_skip_non_critical(
        self, mock_agent, mock_config, sample_findings
    ):
        ...
        assert (
            workflow.progress.stage_results[WorkflowStage.ANALYSIS].status
            == StageStatus.SKIPPED
        )
...
class TestWorkflowIntegration:
    ...
    async def test_workflow_with_real_strategy(self, mock_agent, mock_config):
        ...
        plan = workflow.strategy.create_research_plan(
            "machine learning algorithms", {"strategy": "standard"}
        )
...
async def test_workflow_end_to_end(mock_agent, mock_config, sample_findings):
    ...
    findings = await workflow.execute_research_pipeline(
        keyword="artificial intelligence", strategy="standard", max_retries=2
    )
