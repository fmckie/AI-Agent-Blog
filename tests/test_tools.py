    ...
    async def test_successful_search(self, mock_config):
        ...
        async with TavilyClient(mock_config) as client:
            ...
            with patch.object(client.session, "post") as mock_post:
                ...
                assert (
                    result.answer
                    == "Academic research shows significant findings in this area."
                )

                # Check that results are sorted by credibility
                credibility_scores = [
                    r.credibility_score for r in result.results if r.credibility_score
                ]
                ...
                assert (
                    len(academic_results) >= 2
                )  # Should have at least .edu and .gov results
    ...
    async def test_credibility_scoring(self, mock_config):
        ...
        async with TavilyClient(mock_config) as client:
            ...
            edu_result = {
                ...
                "content": "research study findings",
            }
            gov_result = {
    ...
    async def test_search_academic_sources(self, mock_config):
        ...
        with patch("tools.TavilyClient") as MockClient:
            ...
            mock_client.search = AsyncMock(
                return_value=TavilySearchResponse(query="test", results=[], answer=None)
            )
