...
class TestIntegrationScenarios:
    ...
    async def test_full_search_flow_with_academic_results(self, mock_config):
        ...
        async with TavilyClient(mock_config) as client:
            ...
            journal_result = next(
                r for r in result.results if "journal.nature.com" in r.url
            )
            assert (
                journal_result.credibility_score >= 0.8
            )  # Should be high but may not reach 0.9
    ...
    async def test_rate_limit_with_concurrent_requests(self, mock_config):
        ...
        async with client:
            ...
            with patch("asyncio.sleep", side_effect=mock_sleep):
                ...
                mock_response.raise_for_status = AsyncMock()
                mock_response.json = AsyncMock(
                    return_value={"results": [], "answer": None}
                )
