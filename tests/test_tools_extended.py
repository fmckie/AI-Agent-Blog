        ...
        async with TavilyClient(mock_config) as client:
            ...
            mock_response.json = AsyncMock(return_value={
                "results": [
                    ...
                    {"title": "Complete", "url": "http://complete.com", "content": "Content"}
                ]
            })
    ...
    def test_statistics_with_various_formats(self):
        ...
        for text, expected_stats in test_cases:
            ...
            for expected in expected_stats:
                assert any(expected in stat for stat in stats), f"Expected {expected} in {stats}"
...
class TestURLContentExtraction:
    ...
    async def test_extract_url_content_with_redirects(self):
        ...
        final_response.text = AsyncMock(return_value="<html><body>Content</body></html>")
    ...
    async def test_crawl_website_depth_limiting(self):
        ...
        async def mock_get(url, **kwargs):
            response = AsyncMock()
            response.status = 200
            response.text = AsyncMock(return_value=pages.get(url, "<html>Not found</html>"))
            return response
