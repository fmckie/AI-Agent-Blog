...
class TestWorkflowTransactions:
    ...
    async def test_workflow_state_transitions(self, mock_config):
        ...
        with patch("workflow.create_research_agent", return_value=Mock()):
            with patch("workflow.create_writer_agent", return_value=Mock()):
                ...
                # Update with data
                orchestrator._update_state(WorkflowState.RESEARCH_COMPLETE, {
                    "sources_found": 5
                })
    ...
    async def test_state_recovery(self, mock_config):
        ...
        with patch("workflow.create_research_agent", return_value=Mock()):
            with patch("workflow.create_writer_agent", return_value=Mock()):
                ...
                assert orchestrator.temp_output_dir == mock_config.output_dir / "temp_dir"
    ...
    @pytest.mark.asyncio
    async def test_atomic_save_operations(self, mock_config, mock_research_findings, mock_article_output):
    ...
    @pytest.mark.asyncio
    async def test_workflow_failure_triggers_rollback(self, mock_config, mock_research_findings):
        ...
        with patch("workflow.create_research_agent", return_value=Mock()):
            with patch("workflow.create_writer_agent", return_value=Mock()):
                ...
                orchestrator.run_writing = AsyncMock(side_effect=Exception("Writing failed"))
...
class TestWorkflowRecovery:
    ...
    @pytest.mark.asyncio
    async def test_resume_from_research_state(self, mock_config, mock_research_findings, mock_article_output):
        ...
        with patch("workflow.create_research_agent", return_value=Mock()):
            with patch("workflow.create_writer_agent", return_value=Mock()):
                with patch("workflow.run_research_agent", AsyncMock(return_value=mock_research_findings)):
                    with patch("writer_agent.agent.run_writer_agent", AsyncMock(return_value=mock_article_output)):
                        orchestrator = WorkflowOrchestrator(mock_config)
                        orchestrator._save_outputs_atomic = AsyncMock(
                            return_value=mock_config.output_dir / "output" / "index.html"
                        )
    ...
    @pytest.mark.asyncio
    async def test_resume_cleans_up_on_success(self, mock_config, mock_research_findings, mock_article_output):
    ...
    async def test_resume_rollback_on_failure(self, mock_config):
        ...
        with patch("workflow.create_research_agent", return_value=Mock()):
            with patch("workflow.create_writer_agent", return_value=Mock()):
                with patch("workflow.run_research_agent", AsyncMock(side_effect=Exception("Research failed"))):
                    orchestrator = WorkflowOrchestrator(mock_config)
                    orchestrator._rollback = AsyncMock()
...
class TestProgressCallbacks:
    ...
    @pytest.mark.asyncio
    async def test_workflow_reports_progress(self, mock_config, mock_research_findings, mock_article_output):
