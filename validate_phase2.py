...
async def validate_phase2():
    ...
    findings = await run_research_workflow(
        agent, "renewable energy trends 2024", config, progress_callback=track_progress
    )
    ...
    print(
        f"  - Research quality: {'PASS' if len(findings.academic_sources) >= 3 else 'NEEDS IMPROVEMENT'}"
    )
