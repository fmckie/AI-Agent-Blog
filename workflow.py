...
class WorkflowOrchestrator:
                ...
                if temp_dir.is_dir():
                    try:
                        ...
                        if age_hours > older_than_hours:
                            ...
                            cleaned_temp_dirs += 1
                            logger.info(
                                f"Cleaned up old temp directory: {temp_dir.name}"
                            )
                    except Exception as e:
                        logger.error(f"Failed to clean up {temp_dir}: {e}")

            logger.info(
                f"Cleanup complete: {cleaned_state_files} state files, {cleaned_temp_dirs} temp directories"
            )
    ...
    def _update_state(
        self, new_state: WorkflowState, data: Optional[Dict[str, Any]] = None
    ):
        """
        ...
        logger.info(
            f"Workflow state transition: {self.current_state.value} -> {new_state.value}"
        )
    ...
    async def resume_workflow(self, state_file: Path) -> Path:
        ...
        try:
            ...
            if self.current_state == WorkflowState.RESEARCHING:
                ...
                WorkflowState.WRITING_COMPLETE,
            ]:
                ...
                raise ValueError(
                    f"Cannot resume from state: {self.current_state.value}"
                )
            ...
            output_path = await self._save_outputs_atomic(
            ...
            logger.info(
                f"Workflow resumed and completed successfully. Output: {output_path}"
            )
    ...
    async def run_full_workflow(self, keyword: str) -> Path:
        ...
        self.state_file = (
            self.output_dir / f".workflow_state_{safe_keyword}_{timestamp}.json"
        )
        ...
        try:
            ...
            self._report_progress(
                "writing_complete", f"Generated {article.word_count} words"
            )
    ...
    async def run_research(self, keyword: str) -> ResearchFindings:
        ...
        try:
            ...
            self._report_progress(
                "research", f"Analyzing '{keyword}' across academic databases..."
            )
    ...
    async def run_writing(
        ...
        try:
            ...
            self._report_progress(
                "writing",
                f"Creating article from {len(research_findings.academic_sources)} sources...",
            )
