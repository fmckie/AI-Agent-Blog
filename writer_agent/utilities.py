...
def extract_headers_structure(content: str) -> List[Dict[str, Any]]:
    ...
    html_headers = re.findall(r'<h([1-6])>(.*?)</h\1>', content, re.IGNORECASE)
    ...
    md_headers = re.findall(r'^(#{1,6})\s+(.+)$', content, re.MULTILINE)
    for hashes, text in md_headers:
        headers.append({
            "level": len(hashes),
            "text": text.strip(),
            "type": "markdown"
...
def find_transition_words(text: str) -> Dict[str, Any]:
    ...
    transition_words = [
        ...
        'in addition', 'as well as', 'not only', 'but also',
        # Contrast
        'however', 'nevertheless', 'nonetheless', 'on the other hand',
        'in contrast', 'whereas', 'while', 'although', 'despite',
        # Cause/Effect
        'therefore', 'consequently', 'as a result', 'thus', 'hence',
        'accordingly', 'for this reason', 'because', 'since',
        # Example
        ...
        'ultimately', 'in short', 'to conclude'
    ]
    ...
    transition_percentage = (total_transition_count / word_count * 100) if word_count > 0 else 0
    
    return {
        "found": found_transitions,
        "total_count": total_transition_count,
        "percentage": round(transition_percentage, 2),
        "recommendation": "Good (10-20%)" if 10 <= transition_percentage <= 20 else "Consider adding more transitions"
    }
...
def analyze_keyword_placement(content: str, keyword: str) -> Dict[str, Any]:
    ...
    while True:
        ...
        if pos == -1:
            break
        placement["positions"].append(pos)
        start = pos + 1
